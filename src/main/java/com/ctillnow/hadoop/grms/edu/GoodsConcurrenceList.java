...
public class GoodsConcurrenceList extends Configured implements Tool {
    ...
    public static class GoodsConcurrenceListMapper extends Mapper<LongWritable,Text,Text,NullWritable> {
        @Override
        protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            String infos[] = value.toString().split("[\t]");
            String strings[] = infos[1].split(",");
            for(String g1:strings)
                for(String g2:strings)
                    context.write(new Text(g1+"\t"+g2),NullWritable.get());
